(function() {
    ...
    // Допоміжна функція для порівняння доменів
    function isAlreadyOnTargetServer(targetUrl) {
        try {
            var currentHost = window.location.host; // наприклад, lampa.mx
            var targetHost = new URL(targetUrl).host; // витягуємо домен з посилання
        ...
        try {
            ...
            $('#REDIRECT').on('hover:enter hover:click hover:touch', function() {
                ...
                if (targetServer && !isAlreadyOnTargetServer(targetServer)) {
                    ...
                    Lampa.Noty.show('Ви вже знаходитесь на цьому сервері');
                }
            });
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: 'Адреса сервера',
                    description: 'Оберіть сервер. Змінити його можна буде кнопкою у верхньому барі'
                },
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: 'Постійний редирект',
                    description: 'Щоб вимкнути постійний редирект, затисніть клавішу ВНИЗ під час завантаження додатка' 
                }
            });

            // Слухач для скасування
            Lampa.Keypad.listener.follow("keydown", function (e) {
                 if (e.code === 40 || e.code === 29461) {
                    Lampa.Storage.set('const_redirect', false);
                    if (redirectTimerId) {
                        clearTimeout(redirectTimerId);
                        Lampa.Noty.show('Постійний редирект скасовано!');
                    }
                 } 
            });

            // Логіка постійного редиректу
            if(Lampa.Storage.field('const_redirect') === true) {
                var targetServer = Lampa.Storage.get('location_server');
                
                if (targetServer && targetServer !== '' && !isAlreadyOnTargetServer(targetServer)) {
                    Lampa.Noty.show('Редирект через 3 сек. Натисніть ВНИЗ для скасування.');
                    
                    redirectTimerId = setTimeout(function () {
                        window.location.href = targetServer;
                    }, 3000); // 3 секунди на скасування
                }
            }
